...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        //先利用选课记录表中的id进行幂等性判断，订单表中无记录后，插入订单表：订单主表，订单明细表
        XcOrders xcOrders = saveXcOrders(userId, addOrderDto);

        //插入支付记录
        XcPayRecord payRecord = createPayRecord(xcOrders);
        //获得支付记录表的“本系统支付交易号”，此号码为支付记录表中的支付交易好，非订单表
    ...
    public PayStatusDto queryPayResultFromAlipay(String payNo){
        ...
        bizContent.put("out_trade_no", payNo);//payNO为需要查询的订单号
    ...
    @Override
    @Transactional
    public void saveAliPayStatus(PayStatusDto payStatusDto){//支付成功：更新支付记录表的状态为支付成功，更新订单表的状态为支付成功
        //支付记录号
    ...
    public XcPayRecord createPayRecord(XcOrders orders){
        ...
        if("600002".equals(status)){//支付成功   601001 未支付    601003 已退款
    ...
    public XcOrders saveXcOrders(String userId, AddOrderDto addOrderDto){
        ...
        xcOrders.setStatus("600001");//未支付 --已支付
        xcOrders.setUserId(userId);
        xcOrders.setOrderType("60201");//订单类型：购买课程  --学习资料
    ...
    /**
     * 根据业务id查询订单 ,业务id是选课记录表中的主键，同一个选课记录，只能有一个订单，此id在数据库中做了唯一性约束
     * @param businessId  选课记录表中的主键
     * @return
     */
